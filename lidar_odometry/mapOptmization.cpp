...
public:
    ...
    mapOptimization()
    {
        ...
        subLaserCloudInfo = this->create_subscription<lvi_sam_msgs::msg::CloudInfo>(
            PROJECT_NAME + "/lidar/feature/cloud_info", rclcpp::SensorDataQoS(),
            std::bind(&mapOptimization::laserCloudInfoHandler, this, _1));
        subGPS = this->create_subscription<sensor_msgs::msg::NavSatFix>(
            gpsTopic, rclcpp::SensorDataQoS(),
            std::bind(&mapOptimization::gpsHandler, this, _1));
        subLoopInfo = this->create_subscription<std_msgs::msg::Float64MultiArray>(
            PROJECT_NAME + "/vins/loop/match_frame", rclcpp::SensorDataQoS(),
            std::bind(&mapOptimization::loopHandler, this, _1));
    ...
    void init(){
        tfBroadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(shared_from_this());
    }
    ...
    {
        // RCLCPP_INFO(this->get_logger(), "Received GPS data in mapOptimization");
        std::lock_guard<std::mutex> lock(mtx);
    ...
    pcl::PointCloud<PointType>::Ptr transformPointCloud(pcl::PointCloud<PointType>::Ptr cloudIn, PointTypePose* transformIn)
    {
        ...
        Eigen::Affine3f transCur = getTransform(transformIn->x, transformIn->y, transformIn->z, transformIn->roll, transformIn->pitch, transformIn->yaw);
    ...
    { 
        return getTransform(thisPoint.x, thisPoint.y, thisPoint.z, thisPoint.roll, thisPoint.pitch, thisPoint.yaw);
    }

    Eigen::Affine3f trans2Affine3f(float transformIn[])
    {
        return getTransform(transformIn[3], transformIn[4], transformIn[5], transformIn[0], transformIn[1], transformIn[2]);
    }
    ...
    void performLoopClosure(const std_msgs::msg::Float64MultiArray& loopMsg)
    {
        ...
        Eigen::Affine3f pose_diff_t; // serves as initial guess
        {
            ...
            if (t_diff.norm() < historyKeyframeSearchRadius)
                t_diff.setZero();
            pose_diff_t = getTransform(t_diff.x(), t_diff.y(), t_diff.z(), 0, 0, 0);
        }
    ...
    {        
        ...
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            lastImuTransformation = getTransform(0, 0, 0, cloudInfo.imu_roll_init, cloudInfo.imu_pitch_init, cloudInfo.imu_yaw_init); // save imu before return;
            return;
        }
        ...
        if (cloudInfo.odom_available == true && cloudInfo.odom_reset_id == odom_reset_id)
        {
            ...
            if (lastVinsTransAvailable == false)
            {
                ...
                lastVinsTransformation = getTransform(cloudInfo.odom_x,    cloudInfo.odom_y,     cloudInfo.odom_z, 
                ...
                Eigen::Affine3f transBack = getTransform(cloudInfo.odom_x,    cloudInfo.odom_y,     cloudInfo.odom_z, 
                                                                   cloudInfo.odom_roll, cloudInfo.odom_pitch, cloudInfo.odom_yaw);
        ...
        if (cloudInfo.imu_available == true)
        {
            ...
            Eigen::Affine3f transBack = getTransform(0, 0, 0, cloudInfo.imu_roll_init, cloudInfo.imu_pitch_init, cloudInfo.imu_yaw_init);
    ...
    void transformUpdate()
    {
        if (cloudInfo.imu_available == true)
        {
            if (std::abs(cloudInfo.imu_pitch_init) < 1.4)
            {
                ...
                tf2::Quaternion blendedQuaternion = transformQuaternion.slerp(imuQuaternion, imuWeight);
                tf2::Matrix3x3(blendedQuaternion).getRPY(rollMid, pitchMid, yawMid);
                transformTobeMapped[0] = rollMid;

                // slerp pitch
                transformQuaternion.setRPY(0, transformTobeMapped[1], 0);
                imuQuaternion.setRPY(0, cloudInfo.imu_pitch_init, 0);
                blendedQuaternion = transformQuaternion.slerp(imuQuaternion, imuWeight);
                tf2::Matrix3x3(blendedQuaternion).getRPY(rollMid, pitchMid, yawMid);
    ...
    bool saveFrame()
    {
        ...
        Eigen::Affine3f transFinal = getTransform(transformTobeMapped[3], transformTobeMapped[4], transformTobeMapped[5], 
                                                            transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]);
    ...
    void addGPSFactor()
    {
        ...
        while (!gpsQueue.empty())
        {
            ...
            else
            {
                ...
                float noise_y = thisGPS.position_covariance[7];
                float noise_z = thisGPS.position_covariance[14];
                if (noise_x > gpsCovThreshold || noise_y > gpsCovThreshold)
                    continue;

                float gps_x = thisGPS.latitude;
                float gps_y = thisGPS.longitude;
    ...
    void publishFrames()
    {
        ...
        if (pubCloudRegisteredRaw->get_subscription_count() != 0)
        {
            ...
            *cloudOut = *transformPointCloud(cloudOut,  &thisPose6D);
            // RCLCPP_INFO(this->get_logger(), "Publishing data to /lidar/mapping/cloud_registered_raw");
...
int main(int argc, char** argv)
{
    ...
    RCLCPP_INFO(node->get_logger(), "\033[1;32m----> Lidar Map Optimization Started.\033[0m");
    
    std::thread loopDetectionthread(&mapOptimization::loopClosureThread, node.get());
