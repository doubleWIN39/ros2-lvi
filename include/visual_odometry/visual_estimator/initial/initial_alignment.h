...
public:
    ...
    odometryRegister(ros::NodeHandle n_in):
        n(n_in)
#else
    odometryRegister(rclcpp::Node::SharedPtr n_in, const Eigen::Matrix3d& R_lidar_imu_,
...
#endif
    {
        ...
        q_lidar_to_cam = tf2::Quaternion(q_lidar_imu.x(), q_lidar_imu.y(), q_lidar_imu.z(), q_lidar_imu.w());  
    #endif
    }

    // convert odometry from ROS Lidar frame to VINS camera frame  将里程计从ROS激光帧转换到VINS相机帧
    vector<float> getOdometry(deque<nav_msgs::msg::Odometry>& odomQueue, double img_time)
    {
        ...
        // pop old odometry msg  从 odomQueue 中删除时间戳早于 img_time - 0.05 的消息，以确保队列中的数据与图像时间相近。
        ...
        // 将 Odometry 数据的旋转从 LiDAR 的坐标系转换到相机的坐标系,只进行了旋转，假设雷达相机和iMU彼此足够近
        ...
        tf2::Quaternion q_odom_cam = q_odom_lidar * q_lidar_to_cam; // global rotate by pi // mark: camera - lidar
    #endif
        tf2::fromMsg(odomCur.pose.pose.orientation, q_odom_cam);
