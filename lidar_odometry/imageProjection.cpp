...
private:
    ...
    rclcpp::Publisher<lvi_sam_msgs::msg::CloudInfo>::SharedPtr pubLaserCloud;
    ...
    Eigen::Affine3f getTransform(float x, float y, float z, float roll, float pitch, float yaw) {
...
public:
    ...
    deskewFlag(0)
    {
        subImu = this->create_subscription<sensor_msgs::msg::Imu>(
            ...
            std::bind(&ImageProjection::imuHandler, this, _1));

        subOdom = this->create_subscription<nav_msgs::msg::Odometry>(
            PROJECT_NAME + "/vins/odometry/imu_propagate_ros", rclcpp::SensorDataQoS(),
            std::bind(&ImageProjection::odometryHandler, this, _1));

        subLaserCloud = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            pointCloudTopic, rclcpp::SensorDataQoS(),
            std::bind(&ImageProjection::cloudHandler, this, _1));

        pubExtractedCloud = this->create_publisher<sensor_msgs::msg::PointCloud2> (PROJECT_NAME + "/lidar/deskew/cloud_deskewed", 5);
        pubLaserCloudInfo = this->create_publisher<lvi_sam_msgs::msg::CloudInfo> (PROJECT_NAME + "/lidar/deskew/cloud_info", 5);
    ...
    void imuHandler(const std::shared_ptr<const sensor_msgs::msg::Imu>& imuMsg)
    {
        // RCLCPP_INFO(this->get_logger(), "Received IMU data in ImageProjection");
        sensor_msgs::msg::Imu thisImu = imuConverter(*imuMsg);
        std::lock_guard<std::mutex> lock1(imuLock);
        imuQueue.push_back(thisImu);
    }

    void odometryHandler(const std::shared_ptr<const nav_msgs::msg::Odometry>& odometryMsg)
    {
        std::lock_guard<std::mutex> lock2(odoLock);
        odomQueue.push_back(*odometryMsg);
    }

    void cloudHandler(const std::shared_ptr<const sensor_msgs::msg::PointCloud2>& laserCloudMsg)
    {
        // RCLCPP_INFO(this->get_logger(), "Received Lidar data in ImageProjection");
        if (!cachePointCloud(laserCloudMsg)){
            // RCLCPP_WARN(this->get_logger(), "Checkpoint 1 failed");
            return;
        }

        if (!deskewInfo()){
            // RCLCPP_WARN(this->get_logger(), "Checkpoint 2 failed");
            return;
        }

        projectPointCloud();
        // RCLCPP_INFO(this->get_logger(), "projectPointCloud completed");

        cloudExtraction();
        // RCLCPP_INFO(this->get_logger(), "cloudExtraction completed");

        publishClouds();
        // RCLCPP_INFO(this->get_logger(), "publishClouds completed");

        resetParameters();
    }

    bool cachePointCloud(const std::shared_ptr<const sensor_msgs::msg::PointCloud2>& laserCloudMsg)
    {
        // cache point cloud
    ...
    bool deskewInfo()
    {
        // RCLCPP_INFO(this->get_logger(), "Checkpoint 2: Entered");
        std::lock_guard<std::mutex> lock1(imuLock);
        std::lock_guard<std::mutex> lock2(odoLock);
        // RCLCPP_INFO(this->get_logger(), "Checkpoint 2: Locks declared");

        // RCLCPP_INFO(this->get_logger(), "IMU Queue Size: %ld", imuQueue.size());

        // if (!imuQueue.empty()) {
        //     RCLCPP_INFO(this->get_logger(), "IMU front stamp: %.9f", rclcpp::Time(imuQueue.front().header.stamp).seconds());
        //     RCLCPP_INFO(this->get_logger(), "IMU back  stamp: %.9f", rclcpp::Time(imuQueue.back().header.stamp).seconds());
        // }

        // RCLCPP_INFO(this->get_logger(), "Lidar Scan Cur : %.9f", timeScanCur);
        // RCLCPP_INFO(this->get_logger(), "Lidar Scan Next: %.9f", timeScanNext);
        ...
        // RCLCPP_INFO(this->get_logger(), "Checkpoint 2: IMU data available");
    ...
    void odomDeskewInfo()
    {
        ...
        for (int i = 0; i < (int)odomQueue.size(); ++i)
        {
            ...
            if (rclcpp::Time(endOdomMsg.header.stamp).seconds() < timeScanNext)
                continue;
            else
                break;
        }
        ...
        Eigen::Affine3f transBegin = getTransform(startOdomMsg.pose.pose.position.x, startOdomMsg.pose.pose.position.y, startOdomMsg.pose.pose.position.z, roll, pitch, yaw);

        tf2::fromMsg(endOdomMsg.pose.pose.orientation, orientation);
        tf2::Matrix3x3(orientation).getRPY(roll, pitch, yaw);
        Eigen::Affine3f transEnd = getTransform(endOdomMsg.pose.pose.position.x, endOdomMsg.pose.pose.position.y, endOdomMsg.pose.pose.position.z, roll, pitch, yaw);
    ...
    PointType deskewPoint(PointType *point, double relTime)
    {
        ...
        {
            transStartInverse = (getTransform(posXCur, posYCur, posZCur, rotXCur, rotYCur, rotZCur)).inverse();
            firstPointFlag = false;
        }

        // transform points to start
        Eigen::Affine3f transFinal = getTransform(posXCur, posYCur, posZCur, rotXCur, rotYCur, rotZCur);
    ...
    void publishClouds()
    {
        ...
        // RCLCPP_INFO(this->get_logger(), "Publishing data to /lidar/deskew/cloud_deskewed");
        cloudInfo.cloud_deskewed  = publishCloud(pubExtractedCloud, extractedCloud, cloudHeader.stamp, "base_link");
        // RCLCPP_INFO(this->get_logger(), "Publishing data to /lidar/deskew/cloud_info");
...
int main(int argc, char** argv)
{
    ...
    RCLCPP_INFO(node->get_logger(), "\033[1;32m----> Lidar Cloud Deskew Started.\033[0m");
