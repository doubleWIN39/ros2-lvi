...
public:
    ...
    IMUPreintegration()
    {
        ...
        subImu = this->create_subscription<sensor_msgs::msg::Imu>(
            ...
            std::bind(&IMUPreintegration::imuHandler, this, _1));

        subOdometry = this->create_subscription<nav_msgs::msg::Odometry>(
            PROJECT_NAME + "/lidar/mapping/odometry", rclcpp::SensorDataQoS(),
            std::bind(&IMUPreintegration::odometryHandler, this, _1));
    ...
    void init(){
        tfMap2Odom = std::make_shared<tf2_ros::TransformBroadcaster>(this->shared_from_this());
        tfOdom2BaseLink = std::make_shared<tf2_ros::TransformBroadcaster>(this->shared_from_this());
    }
    ...
    void odometryHandler(const std::shared_ptr<const nav_msgs::msg::Odometry>& odomMsg)
    {
        double currentCorrectionTime = rclcpp::Time((*odomMsg).header.stamp).seconds();
        ...
        if (systemInitialized == false)
        {
            ...
            {
                if (rclcpp::Time(imuQueOpt.front().header.stamp).seconds() < currentCorrectionTime - delta_t)
                {
                    lastImuT_opt = rclcpp::Time(imuQueOpt.front().header.stamp).seconds();
                    imuQueOpt.pop_front();
                }
                else
                    break;
        ...
        while (!imuQueOpt.empty())
        {
            ...
            double imuTime = rclcpp::Time((*thisImu).header.stamp).seconds();
        ...
        double lastImuQT = -1;
        while (!imuQueImu.empty() && rclcpp::Time(imuQueImu.front().header.stamp).seconds() < currentCorrectionTime - delta_t)
        {
            lastImuQT = rclcpp::Time(imuQueImu.front().header.stamp).seconds();
            imuQueImu.pop_front();
        }
        // repropogate
        if (!imuQueImu.empty())
    ...
    {
        // RCLCPP_INFO(this->get_logger(), "Received IMU data in IMUPreintegration");
        ...
        gtsam::Quaternion gtsam_q = lidarPose.rotation().toQuaternion();
        tf2::Quaternion tf2_q(gtsam_q.x(), gtsam_q.y(), gtsam_q.z(), gtsam_q.w());
        odometry.pose.pose.orientation = tf2::toMsg(tf2_q);
        ...
        t.header.stamp = thisImu.header.stamp;
        ...
        tfOdom2BaseLink->sendTransform(t);
    }
};
...
int main(int argc, char** argv)
{
    ...
    RCLCPP_INFO(node->get_logger(), "\033[1;32m----> Lidar IMU Preintegration Started.\033[0m");
